...
class ToolHead:
    def __init__(self, config):
        ...
        #hzk123 add 
        #正常模式
        self.normal_max_velocity = self.max_velocity
        self.normal_max_accel = self.max_accel
        self.normal_requested_accel_to_decel = self.max_accel_to_decel
        self.normal_square_corner_velocity = self.square_corner_velocity

        self.limit_max_velocity = 99999999.0
        self.limit_max_accel = 99999999.0
        self.limit_max_accel_to_decel = 99999999.0
        self.limit_square_corner_velocity = 99999999.0
    ...
    #hzk123 add 
    def set_velocity_limitx(self, max_velocity, max_accel, square_corner_velocity, max_accel_to_decel):
        self.limit_max_velocity = max_velocity
        self.limit_max_accel = max_accel
        self.limit_max_accel_to_decel = max_accel_to_decel
        self.limit_square_corner_velocity = square_corner_velocity
        if max_velocity < self.normal_max_velocity:
            self.max_velocity = max_velocity
        else:
            self.max_velocity = self.normal_max_velocity
        if max_accel < self.normal_max_accel:
            self.max_accel = max_accel
        else:
            self.max_accel = self.normal_max_accel
        if max_accel_to_decel < self.normal_requested_accel_to_decel:
            self.max_accel_to_decel = max_accel_to_decel
        else:
            self.max_accel_to_decel = self.normal_requested_accel_to_decel
        if square_corner_velocity < self.normal_square_corner_velocity:
            self.square_corner_velocity = square_corner_velocity
        else:
            self.square_corner_velocity = self.normal_square_corner_velocity
        self._calc_junction_deviation()
        msg = ("max_velocity: %.6f\n"
               "max_accel: %.6f\n"
        ...
        self.printer.set_rollover_info("toolhead", "toolhead: %s" % (msg,), False)
    #hzk123 add end
    ...
    def cmd_SET_VELOCITY_LIMIT(self, gcmd):
        ...
        if max_velocity is not None:
            self.normal_max_velocity = max_velocity
            if max_velocity <= self.limit_max_velocity: #hzk123 add
                self.max_velocity = max_velocity
        if max_accel is not None:
            self.normal_max_accel = max_accel
            if max_accel <= self.limit_max_accel: #hzk123 add
                self.max_accel = max_accel
        if square_corner_velocity is not None:
            self.normal_square_corner_velocity = square_corner_velocity
            if square_corner_velocity <= self.limit_square_corner_velocity: #hzk123 add
                self.square_corner_velocity = square_corner_velocity
        if requested_accel_to_decel is not None:
            self.normal_requested_accel_to_decel = requested_accel_to_decel
            if requested_accel_to_decel <= self.limit_max_accel_to_decel: #hzk123 add
